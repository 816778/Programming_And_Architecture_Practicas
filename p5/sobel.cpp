...
void createBuffers(cl_context context, cl_mem* in_buffer, cl_mem* out_buffer, cl_mem* sobel_x_buffer, cl_mem* sobel_y_buffer, 
    ...
    *sobel_y_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, 3 * 3 * sizeof(float), NULL, &err);
    ...
    // Transfer Sobel arguments to the device memory
...
int main(int argc, char** argv) {
    ...
    double kernel_execution_time = (end_time - start_time) * 1e-9; // Convert nanoseconds to seconds
    ...
    double throughput = total_pixels / kernel_execution_time; // Pixels per second
    std::cout << "Kernel throughput: " << throughput << " pixels/second\n";


    size_t memory_footprint = sizeof(float) * VECTOR_SIZE     // Input buffer
                            + sizeof(float) * VECTOR_SIZE     // Output buffer
                            + 3 * 3 * sizeof(float) * 2;      // Sobel X and Sobel Y buffers
