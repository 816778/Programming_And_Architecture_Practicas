...
void multiplicar_matrix(unsigned int size, double min_value, double max_value) {
    ...
    // result.print();  // Comentar para evitar impresión de matrices grandes
}
...
int main(int argc, char* argv[]) {
    ...
    // Medir tiempo de ejecución con clock()
    clock_t start = clock();  // Inicia el cronómetro

    multiplicar_matrix(size, min_value, max_value);  // Ejecución del código que queremos medir

    clock_t end = clock();  // Detiene el cronómetro

    double elapsed_time = double(end - start) / CLOCKS_PER_SEC;  // Convertir ticks a segundos
